...
void make_tcp_rst(uint8_t *packet_s, struct tcp_structure *packet, struct libnet_ethernet_hdr *eth, struct libnet_ipv4_hdr *iph, struct libnet_tcp_hdr *tcph, int tcp_sl, uint32_t way) {

  uint32_t tcp_seq = htonl(ntohl(tcph->th_seq) + tcp_sl);
  packet->eth_hdr = *eth;
  packet->ip_hdr = *iph;
  packet->tcp_hdr = *tcph;
  ...
  u_char* ip_ptr = (u_char*)((void*) &packet->ip_hdr) ;

  memcpy(&packet->tcp_hdr.th_seq , &tcp_seq , 4);
  memcpy(&packet->tcp_hdr.th_ack , &tcph->th_ack , 4);

  packet->ip_hdr.ip_len = htons(0x0028);
  //memcpy(fake_mac, eth->ether_dhost,ETHER_ADDR_LEN);
  //memcpy(&packet->eth_hdr.ether_shost, fake_mac, ETHER_ADDR_LEN);
  memcpy(&packet->eth_hdr.ether_shost, attacker_mac, ETHER_ADDR_LEN);
  if(way == RST_BK) {
  	memcpy(&packet->eth_hdr.ether_dhost, &eth->ether_shost, ETHER_ADDR_LEN);
    memcpy(&packet->ip_hdr.ip_src , &iph->ip_dst, IP_ADDR_LEN);
    memcpy(&packet->ip_hdr.ip_dst , &iph->ip_src, IP_ADDR_LEN);
    packet->tcp_hdr.th_dport = tcph->th_sport;
    packet->tcp_hdr.th_sport = tcph->th_dport;
...
void make_tcp_fin(uint8_t *packet_s, struct tcp_structure *packet, struct libnet_ethernet_hdr *eth, struct libnet_ipv4_hdr *iph, struct libnet_tcp_hdr *tcph, int tcp_sl) {
  
  int tcp_seq = htonl(ntohl(tcph->th_seq) + tcp_sl);
  packet->eth_hdr = *eth;
  packet->ip_hdr = *iph;
  packet->tcp_hdr = *tcph;
  packet->ip_hdr.ip_ttl = 0xff;
  ...
  memcpy(&packet->tcp_hdr.th_seq , &tcph->th_ack , 4);
  memcpy(&packet->tcp_hdr.th_ack , &tcp_seq , 4);

  memcpy(packet_s + HEADER_LEN, http_msg, strlen(http_msg));

  //dump(packet_s, sizeof(struct tcp_structure)+strlen(http_msg));
...
void find_data(pcap_t* handle, const uint8_t *packet, struct tcp_structure *tmp_packet1, struct tcp_structure *tmp_packet2, uint8_t *packet_sF, uint8_t *packet_sB, uint8_t *packet_sB_fin) {
  ...
  if(tcph != NULL) {  
    make_tcp_rst(packet_sF, tmp_packet1, eth, iph, tcph, tcp_sl , RST_FW);
    ...
    for(int i = 0; i < 6 ; i++) {
      if(strncmp((const char*)body,http_method[i],strlen(http_method[i])) == 0) {
    	...
    	make_tcp_fin(packet_sB_fin, tmp_packet2, eth, iph, tcph, tcp_sl);
    	//printf("block packet dump\n");
    	//dump(packet_sB,sizeof(struct tcp_structure) + 8);		
    	if(pcap_sendpacket(handle, packet_sB_fin, sizeof(struct tcp_structure) + 8) != 0)
    ...
    if (tmp != 1) {
      if(pcap_sendpacket(handle, packet_sF, sizeof(struct tcp_structure)) != 0)
        {perror("pcap_sendpacket"); free(packet_sF); free(packet_sB); pcap_close(handle); return -1;}

      make_tcp_rst(packet_sB, tmp_packet2, eth, iph, tcph, tcp_sl , RST_BK);
      //printf("packet_sF\n");
      //dump(packet_sF, sizeof(struct tcp_structure));    	
      ...
      if(pcap_sendpacket(handle, packet_sB, sizeof(struct tcp_structure)) != 0)
        {perror("pcap_sendpacket"); free(packet_sF); free(packet_sB); pcap_close(handle); return -1;}
      //printf("Backward\n");
      //dump(packet_sB, sizeof(struct tcp_structure));
    }
  }  	
}
